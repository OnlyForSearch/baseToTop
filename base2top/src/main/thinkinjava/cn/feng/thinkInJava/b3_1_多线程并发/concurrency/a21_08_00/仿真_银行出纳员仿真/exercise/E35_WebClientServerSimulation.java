package cn.feng.thinkInJava.b3_1_多线程并发.concurrency.a21_08_00.仿真_银行出纳员仿真.exercise;
 ...
 * *******************************************************
 *//**
 * 修改BankTellerSimulation.java,使它表示web客户端,向具有固定数量的服务器发送请求
 *
 * 这么做的目标是要确定这个服务器组可以处理负数的大小
 * */
...
Thinking in Java, 4 668 th Edition Annotated Solution Guide
Server 0 terminating
WebClientGenerator terminating
*///:~

/**
 * This is like an inversion of BankTellerSimulation.java, but instead of lacking
 a predefined number of tellers, the number of servers is fixed. We set the load
 (clients per second) by increasing the loadFactor until the number of clients in
 the queue exceeds our chosen limit. The inference engine is simple: if the client
 line is growing then the program has reached peak loadFactor. However, you
 must run the simulation for awhile to find this loadFactor.
 Try changing parameters like serviceTime of clients to simulate different
 scenarios.
 * */