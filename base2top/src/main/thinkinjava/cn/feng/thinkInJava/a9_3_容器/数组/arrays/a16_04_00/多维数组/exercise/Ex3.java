package cn.feng.thinkInJava.a9_3_容器.数组.arrays.a16_04_00.多维数组.exercise;// arrays/Ex3.java
 ...
 * 其初值必须落在另外两个形式参数所指定的区间之间之内,编写第二个方法,打印出第一个方法
 * 所产生的数组,在main()中通过产生不同的容量的数组并打印其内容来测试这两个方法
 */
public class Ex3 {
    ...
    static double[][] createArray(int size1, int size2, double start, double end) {
        ...
        if (size1 == 1)
            ...
            for (int i = 1; i < size1; i++) {
                ...
                for (int j = 0; j < size2; j++) {
                    result[i][j] = start + ((double) j / (size2 - 1)) * (((double) j + (double) i) / (size2 - 1 + i)) *
                            (end - start);
                }
            }
        return result;
